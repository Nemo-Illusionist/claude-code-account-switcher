...
Ответьте `n` — claude запустится как обычно. Если id есть в нескольких аккаунтах, будет тот же нумерованный выбор, что и у `session copy`.

Обратный случай тоже отрабатывается — когда в аккаунте, под которым вы продолжаете, сессия **есть**, но копия в другом аккаунте свежее:

```
В аккаунте 'default' копия этой сессии свежее той, которую вы продолжаете:
  default       только что    60 KB
  work          15ч назад     60 KB

Заменить копию этого аккаунта копией из 'default'? [y/N]
```

*Более старая* копия в другом аккаунте — это просто история, о ней команда молчит. Молчит она и про голый `--resume` без id: он открывает собственный выбор сессий у claude, и вставать перед ним было бы только помехой.
