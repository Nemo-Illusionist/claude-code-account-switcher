...
- **Лимиты по каждому аккаунту**, 5ч и 7д, чтобы выбрать свежий до того, как упрётесь. См. [`usage`](docs/ru/identity.md#сколько-лимита-осталось-usage).
...
| `claude-acc remove <имя>` | Удалить аккаунт — в Корзину, либо `--purge` насовсем |
