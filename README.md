...
| `claude-acc desktop clone-sandbox <name>` | Clone the Cowork sandbox images into a profile, copy-on-write (macOS/APFS) |
...
claude-acc desktop clone-sandbox work  # clone the ~10 GB Cowork images — free on APFS
...
- **Disk.** Isolation is total, so a profile would re-download the ~10 GB of Cowork sandbox images. `clone-sandbox` makes that free on APFS — see below. Caches (~1.5 GB) are still per-profile.
...
### Not paying for the sandbox twice (`clone-sandbox`)

Almost all of a profile's weight is one file: `vm_bundles/claudevm.bundle/rootfs.img`, ~10 GB of Cowork sandbox root filesystem, identical in every profile. A new profile would download its own copy.

On APFS it doesn't have to. `clone-sandbox` clones the images **copy-on-write** — each profile gets a fully independent file that shares blocks with the original until one of them is written:

```
$ claude-acc desktop clone-sandbox work
Cloned 10 sandbox image(s), 10.1 GB logical, from ~/Library/…/Claude/.
Disk actually used: 40 KB.
```

Ten gigabytes, in a third of a second, for forty kilobytes of directory metadata. `--from <profile>` clones from another profile instead of the app's own; `--force` replaces images a profile already has.
...
**What is not cloned, deliberately.** The bundle mixes downloaded images with per-VM identity, and only the first kind may travel:
...
- **Whether the app accepts pre-seeded images is untested.** The filesystem mechanics are verified; the app's reaction is not — nobody has run Cowork in a profile seeded this way. If it misbehaves, delete that profile's `vm_bundles/` and the app fetches its own. Reports either way are welcome in [#92](https://github.com/Nemo-Illusionist/claude-code-account-switcher/issues/92).
