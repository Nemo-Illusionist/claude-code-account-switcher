...
# itself — that reuses the same lookup logic as the terminal chpwd hook
# (no code duplication) — and then activates for $PWD *itself*, rather than
# letting the source do it: `claude-acc run <name>` has already picked an
# account, and re-deriving one from the directory would silently undo it.
_claude_ensure_wrapper() {
...
# Suppress the activation the sourced script does on load, so an account
# chosen by the caller survives it.
CLAUDE_ACC_NO_AUTOACTIVATE=1
[[ -r '$CLAUDE_SWITCH_SCRIPT' ]] && source '$CLAUDE_SWITCH_SCRIPT' >/dev/null 2>&1
unset CLAUDE_ACC_NO_AUTOACTIVATE

# Derive the account from \$PWD only when the caller named none:
# CLAUDE_CONFIG_DIR set means \`run <name>\`, and CLAUDE_ACC_RUN_DEFAULT
# means \`run default\`, where an empty CLAUDE_CONFIG_DIR is the answer
# rather than the absence of one.
if [[ -z "\${CLAUDE_CONFIG_DIR:-}" && -z "\${CLAUDE_ACC_RUN_DEFAULT:-}" ]]; then
    _claude_activate 2>/dev/null
fi
...
# Activate immediately for the current directory — unless the caller asked
# not to. The generated `claude` wrapper sources this script to reuse its
# lookup, but has an account of its own to honour (see _claude_ensure_wrapper).
[[ -n "${CLAUDE_ACC_NO_AUTOACTIVATE:-}" ]] || _claude_activate
