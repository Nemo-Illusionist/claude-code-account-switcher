...
# IDE запускает бинарник напрямую без .zshrc, поэтому env-переменная должна
# быть выставлена самим wrapper'ом на основе CWD проекта.
_claude_ensure_wrapper() {
    local wrapper="$CLAUDE_SWITCH_BIN/claude"

    # Найти настоящий claude (не наш wrapper)
    local real_claude=""
    local _dir
    for _dir in ${(s/:/)PATH}; do
        [[ "$_dir" == "$CLAUDE_SWITCH_BIN" ]] && continue
        if [[ -x "$_dir/claude" ]]; then
            real_claude="$_dir/claude"
            break
        fi
    done
    [[ -z "$real_claude" ]] && return

    # Пересоздать wrapper только если путь изменился
    local stored_path=""
    [[ -f "$wrapper" ]] && stored_path=$(grep '^exec ' "$wrapper" 2>/dev/null | awk '{print $2}' | tr -d '"')
    [[ "$stored_path" == "$real_claude" ]] && return

    cat > "$wrapper" << WRAPPER_EOF
#!/usr/bin/env zsh
# Auto-generated by claude-switch — do not edit manually.
_CLAUDE_SWITCH_DIR="\$HOME/.claude-switch"
_CLAUDE_ACCOUNTS="\$_CLAUDE_SWITCH_DIR/accounts"
_CLAUDE_LINKS="\$_CLAUDE_SWITCH_DIR/links"
_CLAUDE_CONFIG_FILE="\$_CLAUDE_SWITCH_DIR/config"

if [[ -z "\$CLAUDE_CONFIG_DIR" ]]; then
    _dir="\$PWD"
    _account=""
    while [[ "\$_dir" != "/" && -n "\$_dir" ]]; do
        _account=\$(grep "^\${_dir}=" "\$_CLAUDE_LINKS" 2>/dev/null | head -1 | cut -d= -f2)
        [[ -n "\$_account" ]] && break
        _dir="\${_dir:h}"
    done
    [[ -z "\$_account" ]] && _account=\$(grep '^default=' "\$_CLAUDE_CONFIG_FILE" 2>/dev/null | cut -d= -f2)
    if [[ "\$_account" != "default" && -n "\$_account" && -d "\$_CLAUDE_ACCOUNTS/\$_account" ]]; then
        export CLAUDE_CONFIG_DIR="\$_CLAUDE_ACCOUNTS/\$_account"
    fi
fi
exec "$real_claude" "\$@"
WRAPPER_EOF
